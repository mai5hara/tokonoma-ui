      ...
      <div className="grid gap-6 md:grid-cols-2">
        {FONT_FAMILIES.map(({ id, label, familyName, className, themeVar }) => (
          ...
          >
            ...
            <p className={`text-lg text-text-primary ${className}`}>{SAMPLE_TEXT}</p>
            <MetaLine>
              {className} · {themeVar}
            </MetaLine>
          </article>
      ...
      <ul className="divide-y divide-border rounded-lg border border-border bg-surface-elevated">
        {FONT_SIZE_SCALE.map(({ token, label, value, className }) => (
          ...
          >
            ...
            <p className={`truncate font-sans text-text-primary ${className}`}>{label}</p>
          </li>
        ))}
      </ul>
    </FoundationSection>
...
export function FontWeightSection() {
  return (
    <FoundationSection title="Font Weight" description="Regular through Bold for hierarchy.">
      <ul className="divide-y divide-border rounded-lg border border-border bg-surface-elevated">
        {FONT_WEIGHTS.map(({ label, token, value, className }) => (
          ...
          >
            ...
            <span className="font-mono text-xs text-text-subtle">{value}</span>
            <p className={`font-sans text-lg text-text-primary ${className}`}>{SAMPLE_TEXT}</p>
          </li>
        ))}
      </ul>
    </FoundationSection>
  ...
  return (
    <div className="min-h-screen bg-surface p-8 text-text-primary">
      <header className="mb-10 space-y-2">
        <h1 className="font-sans text-2xl font-semibold tracking-tight">Typography</h1>
        <p className="text-sm text-text-muted">
          Tokens live in{' '}
          <code className="rounded bg-ink-100 px-1 py-0.5 font-mono text-ink-800">@theme</code>{' '}
          in <code className="font-mono text-ink-700">src/index.css</code>. Fonts:{' '}
